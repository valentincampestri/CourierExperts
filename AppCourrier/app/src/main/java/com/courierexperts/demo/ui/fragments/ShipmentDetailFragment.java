...
public class ShipmentDetailFragment extends Fragment {
    ...
    private void render(ShipmentDetailUiState.Success state) {
        ...
        if (binding.tvCostHeader != null) {
            String formatted = String.format(Locale.getDefault(), "$ %.2f", state.getShipment().cost);
            binding.tvCostHeader.setText(getString(R.string.shipment_cost_label, formatted));
            binding.tvCostHeader.setVisibility(View.VISIBLE);
        }
